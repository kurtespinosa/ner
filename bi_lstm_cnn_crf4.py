...
def main():
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch in utils.iterate_minibatches(X_dev, Y_dev, masks=mask_dev, char_inputs=C_dev, batch_size=batch_size):
            ...
            if output_predict:
                utils.output_predictions(predictions, targets, masks, 'tmp3/dev%d' % epoch, label_alphabet,
                                         is_flattened=False)
    ...
    for batch in utils.iterate_minibatches(X_test, Y_test, masks=mask_test, char_inputs=C_test,
        ...
        test_inst += inputs.shape[0]
        if output_predict:
            utils.output_predictions(predictions, targets, masks, 'tmp4/test%d' % epoch, label_alphabet,
                                     is_flattened=False)
