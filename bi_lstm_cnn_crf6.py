...
- check condition on early stopping
'''
__author__ = 'max'
...
def main():
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        logger.info('Epoch %d (learning rate=%.4f, decay rate=%.4f): ' % (epoch, lr, decay_rate))
        train_err = 0.0
        train_corr = 0.0
        train_total = 0
        train_inst = 0
        ...
        for batch in utils.iterate_minibatches(X_dev, Y_dev, masks=mask_dev, char_inputs=C_dev, batch_size=batch_size):
            ...
            if output_predict:
                utils.output_predictions(predictions, targets, masks, 'tmp6/dev%d' % epoch, label_alphabet,
                                         is_flattened=False)
        ...
        logger.info('dev loss: %.4f, corr: %d, total: %d, acc: %.2f%%' % (
            dev_err / dev_inst, dev_corr, dev_total, dev_corr * 100 / dev_total))
        logger.info('dev_err: %.4f, best_loss: %.4f, best_acc: %.4f, dev_corr: %.4f, dev_total: %.4f, (dev_corr/dev_total): %.4f' %(
            dev_err, best_loss, best_acc, dev_corr, dev_total, dev_corr/dev_total))
    ...
    for batch in utils.iterate_minibatches(X_test, Y_test, masks=mask_test, char_inputs=C_test,
        ...
        test_inst += inputs.shape[0]
        if output_predict:
            utils.output_predictions(predictions, targets, masks, 'tmp6/test%d' % epoch, label_alphabet,
                                     is_flattened=False)
